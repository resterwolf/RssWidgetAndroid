package rsswidget.restwl.com.rsswidget.contentprovider;
...
import static rsswidget.restwl.com.rsswidget.database.DatabaseManager.*;
        ...
        switch (uriMatcher.match(uri)) {
            ...
            case URI_NEWS_ID:
                ...
                if (TextUtils.isEmpty(selection)) {
                    selection = _ID + " = " + newsItemId;
                } else {
                    selection = selection + " AND " + _ID + " = " + newsItemId;
                }
                cursor = databaseManager.queryNewsTable(projection, selection,
                        selectionArgs, sortOrder);
                cursor.setNotificationUri(getContext().getContentResolver(),
            ...
            case URI_FILTERED_NEWS:
                if (TextUtils.isEmpty(sortOrder)) {
                    sortOrder = SHORT_NEWS_TABLE_NAME + "." + PUB_DATE + " DESC";
                }
        ...
        switch (uriMatcher.match(uri)) {
            ...
            case URI_BLACK_LIST_ID:
                ...
                if (TextUtils.isEmpty(selection)) {
                    ...
                    selection = selection + " AND " + _ID + " = " + blackListItemId;
                }
                cnt = databaseManager.deleteBlackListTable(selection, selectionArgs);
                break;
        ...
        if (cursor.moveToFirst()) {
            while (!cursor.isAfterLast()) {
                ...
                if (cursor.isNull(cursor.getColumnIndex(NEWS_ID))) {
                    matrixCursor.newRow().add(_ID, id)
                            .add(TITLE, title)
                            .add(DESCRIPTION, description)
                            .add(PUB_DATE, pubDate)
                            .add(LINK, link);
                }
                cursor.moveToNext();
            }
